"""
Run Novel Method: Query Expansion X-REM
...
from models.novel_method import QueryExpansionXREM
...
def main(args=None):
    print("="*70)
    print(" NOVEL METHOD: QUERY EXPANSION X-REM - FULL DATASET")
    print("="*70)
    ...
    if config['query_expansion']['llm_provider'] == 'openai':
        if not os.getenv('OPENAI_API_KEY'):
            print("\n⚠️  Warning: OPENAI_API_KEY not set!")
            print("Using fallback expansion method instead.")
            config['query_expansion']['llm_provider'] = 'fallback'
    ...
    print("\n" + "="*70)
    print(" STEP 3: INITIALIZE QUERY EXPANSION X-REM")
    print("="*70)
    
    print(f"\n💡 Novel Method Configuration:")
    print(f"   LLM Provider: {config['query_expansion']['llm_provider']}")
    print(f"   LLM Model: {config['query_expansion']['llm_model']}")
    print(f"   Num Expansions: {config['query_expansion']['num_expansions']}")
    print(f"   Top-i: {config['retrieval']['top_i']}")
    print(f"   Top-k: {config['retrieval']['top_k']}")
    
    # Create novel retrieval model
    retrieval_model = QueryExpansionXREM(
        image_encoder=image_encoder,
        text_encoder=text_encoder,
        nli_model=nli_model,
        llm_provider=config['query_expansion']['llm_provider'],
        llm_model=config['query_expansion']['llm_model'],
        num_expansions=config['query_expansion']['num_expansions'],
    ...
    print("\n" + "="*70)
    print(" STEP 4: RUN QUERY EXPANSION RETRIEVAL ON TEST SET")
    print("="*70)
    
    predictions = []
    references = []
    
    print(f"\n🔍 Running query expansion retrieval on {len(test_dataloader.dataset)} test samples...")
    print("💡 This uses LLM to expand queries for better retrieval")
    print(f"⚙️  Settings: top_i={config['retrieval']['top_i']}, top_k={config['retrieval']['top_k']}")
    print(f"⚙️  Expansions per query: {config['query_expansion']['num_expansions']}")
    
    for batch in tqdm(test_dataloader, desc="Retrieving with expansion"):
